...
export default function SuppliersPage() {
  ...
  const { register, handleSubmit, reset, setValue } = useForm({
    defaultValues: { openingBalanceType: "CREDIT", openingBalance: 0 }
  });
  ...
  const onSubmit = async (data) => {
    ...
    try {
      ...
      reset({ openingBalanceType: "CREDIT", openingBalance: 0 });
      setEditingId(null);
      fetchSuppliers();
    } catch (err) {
          ...
          <div className="bg-slate-800 border border-slate-700 rounded-xl shadow-sm overflow-x-auto">
            <table className="w-full text-sm text-left text-slate-300 min-w-[600px]">
              ...
              <tbody className="divide-y divide-slate-700/50">
                {suppliers.length === 0 ? (
                  <tr>
                    <td colSpan="4" className="px-6 py-12 text-center text-slate-500">No suppliers found for this company.</td>
                  </tr>
                ) : (
                  suppliers.map(s => (
